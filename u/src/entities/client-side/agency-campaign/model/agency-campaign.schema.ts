import { z } from "zod";
import type { AgencyCampaignCategory } from "./agency-campaign.types";

const REQUIRED_FIELDS: Record<AgencyCampaignCategory, string[]> = {
    artist: ["Campaign objective", "Content available", "Your budget"],
    music: ["Campaign objective", "Content available", "Your budget"],
    event: [
        "What you’re promoting (Campaign Goal)",
        "Content available",
        "Your budget",
    ],
    other: ["Brief"],
...
const baseSchema = z.object({
    ...
    ["Your budget currency"]: z.string().optional(),
    ...
    ["Any extra notes (messaging, influencer type, content ideas)"]: z.string().optional(),

    ["Brief"]: z.string().optional(),
});

export const buildAgencyCampaignSchema = (category: AgencyCampaignCategory) =>
    baseSchema.superRefine((values, ctx) => {
        for (const field of REQUIRED_FIELDS[category]) {
            if (field === "Your budget") continue;

            if (isEmpty(values[field as keyof typeof values])) {
                ctx.addIssue({
