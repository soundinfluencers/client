...
export const ClientCompanySelect: FC<CompanySelectProps> = ({
  selectedType,
  selectType,
  isMenuOpen,
  setIsMenuOpen,
  ...
  return (
    <div className="client-company-select" ref={dropdownRef}>
      ...
      <div
        className={`client-company-select__main ${isMenuOpen ? "client-company-select__main--active" : ""}`}
        onClick={() => setIsMenuOpen(!isMenuOpen)}>
        {selectedType ? (
          <p className="client-company-select__selected-type">{selectedType}</p>
        ) : (
          <p className="client-company-select__placeholder">
            Choose company type
          </p>
        )}
      ...
      <div
        className={`client-company-select__dropdown ${isMenuOpen ? "client-company-select__dropdown--visible" : ""}`}>
        <div className="client-company-select__dropdown-list">
          {clientCompanyTypes.map((companyType) => (
            <div
              key={companyType}
              className="client-company-select__dropdown-list-item"
              onClick={() => selectType(companyType)}>
              <p>{companyType}</p>
