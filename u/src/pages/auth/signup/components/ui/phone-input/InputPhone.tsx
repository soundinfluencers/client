...
export const InputPhone: FC<InputPhoneProps> = ({ value, setValue, isMenuOpen, setIsMenuOpen, isError }: InputPhoneProps) => {
    ...
    return (
        <div className={`input-phone ${!isValid || isError ? 'error' : ''}`}>
            ...
            <div className={`input-phone__dropdown ${isMenuOpen ? 'input-phone__dropdown--visible' : ''}`}>
                <div className='input-phone__dropdown-search'>
                    <img src={smallSearch} alt='' className='input-phone__dropdown-search-icon' />
                    <input
                ...
                <div className='input-phone__dropdown-list'>
                    {filteredPhoneNumbers.map((phone: PhoneNumberType) => (
                        <div className='input-phone__dropdown-list-item' key={phone.countryName} onClick={() => setSelectedPhoneDetails(phone)}>
                            <div className='input-phone__dropdown-list-item-details'>
                                ...
                                <p className='input-phone__dropdown-list-item-country'>
                                    {phone.countryName} ({phone.countryFlag ? phone.placeholder : ''})
                                </p>
                            </div>
                            {selectedPhoneDetails.countryName === phone.countryName && (
                                <img src={check} alt='checked' className='input-phone__dropdown-list-item-check' />
                            )}
                        </div>
                    ))}
                </div>
