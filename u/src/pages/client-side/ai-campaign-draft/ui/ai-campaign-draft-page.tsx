...
export const AiCampaignDraftPage = () => {
  ...
  const [editingAccountKey, setEditingAccountKey] = React.useState<
    string | null
  >(null);
  const [detailsForm, setDetailsForm] =
    React.useState<DetailsForm>(EMPTY_DETAILS);
  ...
  const editingAccount = React.useMemo(
    () =>
      accounts.find((account) => accountKey(account) === editingAccountKey) ??
      null,
  ...
  const persistDraft = React.useCallback(
    (
      ...
      if (signature === lastQueuedPayloadRef.current)
  ...
  const closeDetails = React.useCallback(
    (force = false) => {
      if (
        !force &&
        detailsAreDirty &&
        !window.confirm("Discard unsaved content changes?")
      )
        return;
  ...
  const openDetails = React.useCallback(
    (account: DraftAddedAccountDto, trigger: HTMLButtonElement) => {
      if (!draft) return;

      const form = getDetailsForm(getContentForAccount(draft, account));
      detailsTriggerRef.current = trigger;
      detailsInitialRef.current = JSON.stringify({
        detailsForm: form,
        applyToAll: false,
      });
      setDetailsForm(form);
  ...
  const updateDetailsField = React.useCallback(
    ...
    (key: string) => {
      ...
      const contentStillUsed = accounts.some(
        (account, accountIndex) =>
          accountIndex !== index &&
          String(
            getSelectedContentRef(account)?.campaignContentItemId ?? "",
          ) === removedContentId,
      );
      setAccounts((current) =>
        current.filter((account) => accountKey(account) !== key),
      );
        ...
        ({ closeToast }) => (
          <div className={styles.undoToast}>
            ...
            <button
              ...
              onClick={() => {
                setAccounts((current) => {
                  if (current.some((account) => accountKey(account) === key))
                    return current;
                  const next = [...current];
  ...
  const toggleActionMenu = React.useCallback(
    (key: string, trigger: HTMLButtonElement) => {
  ...
  const saveAndNavigate = React.useCallback(
    async (path: string, requireSelection = true) => {
      ...
      if (!saved) {
        toast.error(
          "Could not save the draft. Please retry before continuing.",
        );
        return;
      }
      ...
      <main className={styles.page}>
        <div className={styles.header}>
          <div>
            ...
            <p>
              Review the AI shortlist. Uncheck a page to exclude it without
              removing it.
            </p>
          </div>
          <div
        ...
        <section className={styles.draftCard}>
          <div className={styles.tableHeader}>
            <div>
              ...
              <p>
                {selectedAccounts.length} of {accounts.length} included · live
                prices &amp; reach
              </p>
            </div>
            <button
            ...
            <table>
              ...
              <tbody>
                {accounts.map((account) => {
                  ...
                  const isSelected =
                    isAvailable && account.isSelected !== false;
                  ...
                  return (
                    <tr
                      key={key}
                      className={isSelected ? "" : styles.disabledRow}
                    >
                      <td>
                        <input
                          type="checkbox"
                      ...
                      <td>
                        <div className={styles.network}>
                          ...
                          <div>
                            ...
                            <span>
                              {normalizePlatform(account.socialMedia)}
                            </span>
                            <span
                              className={`${styles.contentStatus} ${styles[`contentStatus_${isAvailable ? contentStatus : "incomplete"}`]}`}
                            >
                              {isAvailable
                      ...
                      <td className={styles.price}>
                        {formatCurrency(Number(account.price ?? 0))}
                      </td>
                      <td>
                      ...
                      <td>
                        <div className={styles.rowActions}>
                          ...
                          <button
                            ...
                            onClick={(event) =>
                              toggleActionMenu(key, event.currentTarget)
                            }
                            data-draft-actions
                          >
        ...
        {selectedAccounts.length > 0 && (
          <label
            className={`${styles.contentOption} ${noContentAvailable ? styles.contentOptionChecked : ""}`}
          >
            <input
              type="checkbox"
            ...
            <span>
              ...
              <small>
                {noContentAvailable
                  ? "Selected — our team will prepare the content. You can continue now."
          ...
          <button
            ...
            onClick={() =>
              void saveAndNavigate(
                `/client/create-campaign?draftId=${draft._id}&source=ai`,
              )
            }
          >
            Customize campaign
        ...
        >
          ...
          >
            ...
            <div className={styles.drawerBody}>
              ...
              <label className={styles.field}>
                ...
                <input
                  ...
                  onChange={(event) =>
                    updateDetailsField("contentUrl", event.target.value)
                  }
                ...
                {detailsErrors.contentUrl && (
                  <small role="alert">{detailsErrors.contentUrl}</small>
                )}
              </label>
              ...
              <label className={styles.field}>
                ...
                <textarea
                  ...
                  onChange={(event) =>
                    updateDetailsField("description", event.target.value)
                  }
              ...
              <div className={styles.fieldRow}>
                <label className={styles.field}>
                  ...
                  <input
                    ...
                    onChange={(event) =>
                      updateDetailsField("storyTag", event.target.value)
                    }
                    placeholder="@artist"
                    maxLength={100}
                  />
                ...
                <label className={styles.field}>
                  ...
                  <input
                    ...
                    onChange={(event) =>
                      updateDetailsField("storyLink", event.target.value)
                    }
                    placeholder="Paste a link or reference"
                    maxLength={2000}
                  ...
                  {detailsErrors.storyLink && (
                    <small role="alert">{detailsErrors.storyLink}</small>
                  )}
                </label>
              </div>
              ...
              <label className={styles.field}>
                ...
                <textarea
                  ...
                  onChange={(event) =>
                    updateDetailsField("additionalBrief", event.target.value)
                  }
            ...
            <footer className={styles.drawerFooter}>
              <span>
                {detailsAreDirty ? "Unsaved changes" : "No unsaved changes"}
              </span>
              <div>
                <button
                  type="button"
