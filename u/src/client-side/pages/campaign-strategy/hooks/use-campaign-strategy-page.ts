...
import {
  getCampaignSelectedAccounts,
  groupCampaignContent,
} from "../model/campaign-strategy.helpers";
...
export const useCampaignStrategyPage = () => {
    ...
    const strategyAccounts = React.useMemo(
        () => getCampaignSelectedAccounts(undefined, store),
        [store.offer, store.promoCard],
    );

    const { mainPromos, musicPromos, otherPromos } = useGroupPromos<ConnectedAccount>(strategyAccounts);
